...
URL="https://b2b.fjordnansen.com/product-eng-42971-RIFFE-SHORTS-MEN-boxer-shorts.html"
...
def main():
 ...
 for i,q in enumerate(s.select("[data-amount]")):
  ...
  for _ in range(6):
   ...
   p=p.parent
  rows.append({"i":i,"name":q.get("name"),"amount":int(raw),"text":text[:420]})
 print("RIFFE_BOXER_NODES="+json.dumps(rows,ensure_ascii=False),flush=True)
